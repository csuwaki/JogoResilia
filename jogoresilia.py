...
def perdeu():
    print("Cadê a ferramenta? Escolha terrível! No que você estava pensando? Você perdeu! ")
...
Bem-vindo (a), sobrevivente!")
    ...
    local = input(f"Você decidiu sair do seu bunker para explorar a região. À sua direita há uma poça radioativa; à esquerda, uma pilha de ossos; à frente uma pilha de animais mortos. \n\
Para onde você deseja ir? ")

    if local.title() == 'Poça Radioativa': 
    ...
    elif local.title() == 'Pilha De Ossos':
        seg_e = segunda_escolha()
        if seg_e == 1:
            perdeu()
        else:
...
calmamente. Ela parece amigável. Próximo a ela, você enxerga um martelo. O que você faz? \n\
    ...
    elif local.title() == 'Pilha De Animais Mortos':
        qua_e  = quarta_escolha()
        if qua_e == 1:
            perdeu()
        else:
